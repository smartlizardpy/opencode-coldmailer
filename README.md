...
1. **Product interview.** A short conversation that asks human questions — "who was the last
   ...
   "value proposition". It produces an editable brief, including the *observable signals* that
   mean a company is worth contacting.
2. **Find companies.** opencode's web search, driven by those signals. Or paste a list.
   Nothing is researched until you tick it.
3. **Find the right people.** A bounded, robots-aware crawl of each company's own site
   (homepage plus up to six contact/about/team pages), then the model picks who actually decides
   on *this* ask. Every contact carries the URL it came from.
4. **Write.** Short, plain-text emails. Every personalised claim is traceable to a page and a
   verbatim quote.
5. **Review and send.** You approve each one. Sends are capped per day and spaced apart.
6. **Replies.** IMAP matches replies to the thread and drafts a response you can edit or ignore.
...
- Add an opt-out footer, unless you switch it on in Setup. It's off by default.
